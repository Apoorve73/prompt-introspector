...
const PromptIntrospector = () => {
  ...
  useEffect(() => {
    const checkDefaultKey = async () => {
      try {
        const response = await `${backendUrl}/api/default-key`;
        if (response.ok) {
    ...
    try {
      ...
      setReasoning((prev) => [
        ...prev,
        {
          ...
          thought:
            "All input tokens processed. Now generating response using OpenAI GPT model.",
        ...
        >
          ...
          <div style={{ fontSize: "12px", color: "#666", marginTop: "8px" }}>
            <p style={{ margin: "4px 0" }}>
              {" "}
              🚀 <strong> Backend Proxy: </strong> Uses secure backend server to
              call OpenAI API{" "}
            </p>{" "}
            <p style={{ margin: "4px 0" }}>
              {" "}
            ...
            <p style={{ margin: "4px 0" }}>
              {" "}
              ⚡ <strong> Server: </strong> Make sure backend is running on port
              3001{" "}
            </p>{" "}
            {apiKey === "***DEFAULT_KEY***" && (
              <p style={{ margin: "4px 0", color: "#4caf50" }}>
                {" "}
                🌟 <strong> Default Key: </strong> Using API key from server
                environment{" "}
              </p>
            )}{" "}
          </div>{" "}
          ...
          <div style={{ marginBottom: "30px" }}>
            ...
            >
              ...
              {tokens.map((token, index) => (
                <span key={index} style={getTokenStyle(token, index)}>
                  {" "}
                  {token.text}{" "}
                </span>
              ))}{" "}
              ...
              {reasoning.map((step, index) => (
                <div
                  ...
                  style={{
                    padding: "16px",
                    borderBottom:
                      index < reasoning.length - 1 ? "1px solid #eee" : "none",
                    backgroundColor:
