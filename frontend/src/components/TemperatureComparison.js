...
// API call function
const callOpenAIWithTemperature = async (prompt, temperature, apiKey, backendUrl, sessionId) => {
  try {
    const response = await fetch(`${backendUrl}/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a helpful assistant. Provide creative and engaging responses.',
          },
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <button
              ...
              style={{
                padding: '8px 16px',
                backgroundColor: isGenerating ? '#ccc' : getTemperatureColor(customTemperature),
                color: 'white',
                border: 'none',
                borderRadius: '4px',
                cursor: isGenerating ? 'not-allowed' : 'pointer',
        ...
        <div>
            ...
            {Object.entries(responses)
              ...
              .map(([temperature, { response, error }]) => (
                <div
                  key={temperature}
                  style={{
                    border: `2px solid ${getTemperatureColor(parseFloat(temperature))}`,
                    borderRadius: '10px',
                    overflow: 'hidden',
                  }}
                >
                  <div
                    style={{
                      backgroundColor: getTemperatureColor(parseFloat(temperature)),
                      color: 'white',
                      padding: '12px 16px',
                      fontWeight: 'bold',
                      fontSize: '16px',
                  ...
                  >
                    Temperature: {temperature}({getTemperatureLabel(parseFloat(temperature))}){' '}
                  </div>
                  <div
                    style={{
                      padding: '16px',
                  ...
                  >
                    {' '}
                    {error ? (
                      <div style={{ color: '#c00', fontStyle: 'italic' }}>Error: {error} </div>
                    ) : (
                      <SafeMarkdown key={`${temperature}-${Date.now()}`}> {response} </SafeMarkdown>
                    )}{' '}
                  </div>{' '}
                </div>
              ))}{' '}
          ...
          >
            ...
            <ul style={{ margin: '8px 0', paddingLeft: '20px' }}>
              <li>
                {' '}
                <strong> 0.1 - 0.3(Conservative): </strong> More focused, deterministic responses
              </li>
              <li>
                {' '}
                <strong> 0.4 - 0.7(Balanced): </strong> Good balance of creativity and coherence
              </li>
              <li>
                {' '}
                <strong> 0.8 - 1.0(Creative): </strong> More diverse and creative outputs
              </li>
              <li>
                {' '}
                <strong> 1.1 + (Very Creative): </strong> Highly creative but potentially less
                coherent
              </li>
            </ul>{' '}
          </div>{' '}
