...
const generateTokenReasoning = (token, index, allTokens) => {
  ...
  if (token.type === 'word') {
    ...
    } else if (['what', 'why', 'how', 'when', 'where', 'who'].includes(token.text.toLowerCase())) {
      reasoning.thought = `Question word detected. This indicates an interrogative prompt requiring explanatory response.`;
    } else if (['explain', 'describe', 'tell', 'show'].includes(token.text.toLowerCase())) {
...
const TokenIntrospector = ({ apiKey, backendUrl, sessionId }) => {
  // Use the custom hook for OpenAI functionality
  const {
    tokenize,
    generateResponse,
    error: openAIError,
  } = useOpenAI(apiKey, backendUrl, sessionId);
  ...
  const analyzePrompt = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < newTokens.length; i++) {
        ...
        const tokenReasoning = generateTokenReasoning(currentToken, i, newTokens);
    ...
    <>
      ...
      <div style={{ marginBottom: '30px' }}>
        ...
        <button
          ...
          disabled={
            isAnalyzing || !prompt.trim() || (!apiKey.trim() && apiKey !== '***DEFAULT_KEY***')
          }
          style={{
            marginTop: '12px',
            padding: '12px 24px',
      ...
      {tokens.length > 0 && (
        <div style={{ marginBottom: '30px' }}>
          <h3 style={{ marginBottom: '15px', fontSize: '1.3em' }}> 📝 Token Flow Analysis:</h3>{' '}
          <div
            style={{
              padding: '20px',
          ...
          >
            ...
            {tokens.map((token, index) => (
              <span key={index} style={getTokenStyle(token, index, currentTokenIndex)}>
                {' '}
                {token.text}{' '}
              </span>
            ))}{' '}
          ...
          >
            <span style={{ color: '#ff4444', fontWeight: 'bold' }}> ● Currently Processing </span>{' '}
            <span
              style={{
                marginLeft: '20px',
                color: '#228B22',
            ...
            <span style={{ marginLeft: '20px', color: '#999' }}> ● Pending </span>{' '}
            <span style={{ marginLeft: '20px' }}>Total tokens: {tokens.length} </span>{' '}
          </div>{' '}
        </div>
      )}{' '}
      {/* Reasoning Steps */}{' '}
      {reasoning.length > 0 && (
        <div style={{ marginBottom: '30px' }}>
          <h3 style={{ marginBottom: '15px', fontSize: '1.3em' }}> 🧠 AI Reasoning Process:</h3>{' '}
          <div
            style={{
              maxHeight: '500px',
              overflowY: 'auto',
            ...
            {reasoning.map((step, index) => (
              ...
              >
                ...
                <div style={{ marginBottom: '6px', fontSize: '14px' }}>
                  ...
                  {Array.isArray(step.attention) ? step.attention.join(', ') : step.attention}{' '}
                </div>{' '}
      ...
      {streamedResponse && (
        <div style={{ marginBottom: '20px' }}>
          <h3 style={{ marginBottom: '15px', fontSize: '1.3em' }}> ⚡ Live AI Response:</h3>{' '}
          <div
            style={{
              padding: '16px',
          ...
          >
            <SafeMarkdown key={`streaming-${Date.now()}`}> {streamedResponse} </SafeMarkdown>{' '}
      ...
      {output && (
        <div style={{ marginTop: '20px' }}>
          <h3 style={{ marginBottom: '15px', fontSize: '1.3em' }}> ✅ Complete AI Response:</h3>{' '}
          <div
            style={{
              padding: '20px',
